...
const FacetItem: React.FC<FacetItemProps> = ({ title, list, sortType }) => {
	const currentSelection = useTypedSelector(
		(state) => state.products.filter.currentSelection
	);
	...
	return (
		<div
			onDoubleClick={() => navigate(`category/${title}`)}
		...
		>
			...
			<div className={styles.facetCollapse}>
				<div
					className={`${styles.facetCollapseButton} ${visibleList && styles.active}`}
				>
					<MdKeyboardArrowDown />
				</div>
			</div>
